...
func NewRound(opts *RoundOptions) (r *Round) {
	...
	//// timer
	//r.timers = make([]time.TimerWheel, r.options.Timer)
...
// Timer获取一个定时器
func (r *Round) TimerWheel(rn int) *time.TimerWheel {
	return &(r.timerWheels[rn%r.options.Timer])
}
